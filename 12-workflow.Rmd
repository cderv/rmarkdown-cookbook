...
## Import an R script inside a code chunk {#code-chunk-script}

Another situation is when an author wishes to use an existing R script as the content of a code chunk. This could happen for example with a long already existing analysis script or to import some helpers functions contained in another script. 

To use such script, one could source it in a chunk of the R Markdown document
...
However, this would not render in the output the content of `helpers.R` but only the content of the chunk. To succeed, you can use the `code` chunk option^[You can learn more about chunk options in [@xie2015] and [@xie2018]. You will also find other examples in Section \@ref(knitr-options)]

````r
```{r, code = xfun::read_utf8("helpers.R")}`r ''`
```
````

This will read the content of the file and fill the chunk with it before executing the chunk as usual.

Importing a chunk from another R Markdown file is also possible as explained at \@ref(child-documents).
