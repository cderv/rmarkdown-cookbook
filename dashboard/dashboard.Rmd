...
most_viewed <- read_csv("MOST_VIEWED_QA_SO.csv", 
  ...
  mutate(most_viewed_order = row_number())
```
...
full_table <- so_in_gh %>%
  ...
  select(top, urls, id, issue, status) %>%
  arrange(top) %>%
  mutate(
...
### Nb of SO questions reported (Top 500 or not)

```{r all-so}
flexdashboard::valueBox(
  full_table %>% filter(is.na(top)) %>% count() %>% pull(n),
  caption = "Number of SO QA not in TOP 500"
)
```
