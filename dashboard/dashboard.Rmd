---
title: "Dashboard for SO question in Rmarkdown Cookbook"
output: 
...
full_table <- so_in_gh %>%
  ...
  arrange(top) %>%
  mutate(urls = map_chr(urls, ~htmltools::a(.x, href = .x, target="_blank") %>% as.character),
...
full_table %>%
  ...
  labs(
    title = "Nb of SO questions already in GH issues",
    x = NULL,
    fill = "Top SO QA"
  )
```
...
```{r}
rate <- full_table %>% count(ok = !is.na(issue)) %>% filter(ok) %>% pull(n)
flexdashboard::gauge(rate, 0, 500)
```
...
```{r render-full-table}
DT::datatable(full_table %>% select(-issue), escape = FALSE)
```
